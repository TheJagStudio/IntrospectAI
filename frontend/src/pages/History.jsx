...
const History = () => {
	...
	return (
		<div className="flex min-h-screen bg-slate-50 noScrollBars overflow-hidden">
				...
				<div className="mt-24 p-6">
					{loading ? (
						...
						<div className="flex flex-col items-center justify-center h-64">
							...
							<p className="text-gray-400 text-sm">Start a new conversation to see it here</p>
						</div>
					) : (
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{sessions.map((session) => (
								<div key={session?.uuid} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
									<img src={session?.module?.profileImg || "https://via.placeholder.com/50"} alt={session?.module?.name} className="w-full h-fit rounded-lg object-cover mb-4" />
									<div className="flex items-start space-x-4">
										<div className="flex-1">
											...
											<div className="text-xs mb-4 bg-orange-500 rounded-full px-4 py-1 text-white w-fit">{session?.module?.role}</div>
											<p className="text-gray-600 text-sm mb-3">{session?.conversation?.length} messages</p>
											<div className="flex space-x-2">
												<Link to={`/session/${session?.uuid}`} className="flex-1">
													<button className="w-full bg-purple-600 text-white py-2 px-4 rounded-lg hover:bg-purple-700 transition-colors">Continue Chat</button>
												</Link>
												{session?.isReported && (
													<button
														onClick={() => {
												...
												<button onClick={() => handleDelete(session?.uuid)} className="bg-red-100 text-red-600 py-2 px-4 rounded-lg cursor-pointer hover:bg-red-200 transition-colors">
													Delete
												</button>
											</div>
										</div>
			...
			<ReportModal report={selectedReport} isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} />
		</div>
	);
};
