	...
	return (
		<div className="min-h-screen">
			<nav className="container mx-auto px-6 py-4 flex items-center justify-between">
				<Link to="/">
					<p className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-purple-700 bg-clip-text text-transparent">IntrospectAI</p>
				</Link>
				<div className="space-x-8 flex items-center">
					<Link to="/solutions" className="hover:text-purple-700 transition">
						Solutions
					</Link>
					...
					{userData && (
						<Link to="/apps" className="hover:text-purple-700 transition">
							Dashboard
						</Link>
					)}
					{userData && (
						<div className="relative">
							<button onClick={() => setDropdownOpen(!dropdownOpen)} className="hover:text-purple-700 transition flex items-center space-x-2">
								{userData.photoURL && <img src={userData.photoURL} alt="Profile" className="w-10 h-10 rounded-full border border-gray-300" />}
								<span>{userData.displayName}</span>
							</button>
							{dropdownOpen && (
								<div className="absolute right-0 mt-2 w-48 bg-white shadow-md rounded-lg p-2">
									<button onClick={signOut} className="w-full text-left px-4 py-2 text-red-600 hover:bg-gray-100 rounded-lg">
										Log Out
									</button>
								</div>
							)}
